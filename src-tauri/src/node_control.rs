...
// Vendored at: 2026-05-03
//
// Why vendored: pulling the whole `buckyos-api` crate would drag in the
// 30+ member buckyos workspace (kRPC, rbac, ndn-lib, openssl, rusqlite,
// tokio, ...). node_control itself only depends on std + serde + libc,
// so we keep a copy here until buckyos-api is split into a slim SDK
