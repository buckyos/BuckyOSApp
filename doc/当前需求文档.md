...
本文档只描述当前代码已经落地、已经在代码中明确体现，或已纳入当前版本实现范围的行为。目标态规划、尚未进入当前版本的页面和接口不作为验收范围。

当前项目是一个跨平台 App 项目，同一套产品能力需要同时支持 macOS、Windows、Android 和 iOS。本文档中未特别限定平台的功能要求，默认应在上述四个平台保持功能等价；平台差异会在对应章节单独说明。
...
- OOD 绑定状态页展示当前账户是否已绑定 OOD；未绑定时同一路径展示激活入口，支持局域网扫描和手动输入远程 OOD 地址；已绑定时展示进入应用和解绑入口。
...
- 设置页、语言页、关于页。
- 嵌入式 WebView 测试页。
- 桌面端通过独立 WebView 窗口打开外部页面，移动端通过当前 WebView 内的 `/web-container?embedded=1` 打开外部页面。
- `BuckyApi` 注入与宿主能力桥接。
...
- 完整的应用商店运营能力。
- 完整的 OOD 生命周期管理。
- 基于激活码、中继服务、二维码配对或账号云端发现的远程 OOD 激活。
...
- OOD：用户设备节点。当前版本主要负责局域网扫描、手动输入远程地址、打开激活页、读取和解绑 zone 绑定状态。
...
- 展示 OOD 插图。
- 提供“扫描本地网络内的设备”按钮。
- 提供“激活远程设备”按钮。点击后弹出远程 OOD 地址输入弹框。
...
- 打开前将激活 URL 的 `lang` 查询参数设为 App 当前语言（如 `zh`、`zh-TW`），保留其他查询参数和片段，使 node_active 初始化时沿用 App 语言。
- 移动端通过 `/web-container?embedded=1` 在当前 WebView 内打开。
- 桌面端通过独立 `WebviewWindow` 打开，不在主窗口内跳转，也不替换当前主 App 页面。
- 桌面端窗口标题优先使用设备名或 IP；窗口 label 使用设备名、设备类型和 IP 组合生成，同一 label 已存在时复用并聚焦已有窗口。

### 9.3 远程 OOD 地址输入弹框

页面目标：

- 允许用户在无法通过局域网扫描发现 OOD 时，手动输入远程 OOD 根地址，并继续打开该设备返回的激活页。

入口行为：

- 用户在未绑定 OOD 状态点击“激活远程设备”。
- App 在当前页面弹出输入弹框，不跳转到新路由。
- 弹框包含一个协议下拉框、一个地址输入框、确认按钮和取消按钮。
- 协议下拉框与地址输入框视觉上连接为一个输入控件组，协议下拉在左，地址输入在右。
- 协议下拉框可选择 `http://` 或 `https://`，默认选择 `http://`。
- 取消后关闭弹框，不改变当前页面状态。

地址输入规则：

- 地址输入框的输入值表示 OOD 的 host 和可选端口，当前版本不要求用户输入 `/device` 或激活页路径。
- 支持域名：`example.com`。
- 支持域名加端口：`example.com:3182`。
- 支持 IPv4：`192.168.1.10`。
- 支持 IPv4 加端口：`192.168.1.10:3182`。
- 协议只能通过前置下拉框选择，地址输入框内不应输入 `http://` 或 `https://`。
- 未输入端口时默认使用 `3182`。
- 如果输入了端口，端口必须是 `1` 到 `65535` 之间的整数。
- 当前版本只支持根地址输入；包含 path、query 或 hash 的输入应提示用户只填写设备地址。

地址归一化：

- 协议选择 `http://` 且地址输入 `example.com` 时，归一化为 `http://example.com:3182`。
- 协议选择 `http://` 且地址输入 `example.com:8080` 时，归一化为 `http://example.com:8080`。
- 协议选择 `http://` 且地址输入 `192.168.1.10` 时，归一化为 `http://192.168.1.10:3182`。
- 协议选择 `https://` 且地址输入 `example.com` 时，归一化为 `https://example.com:3182`。

确认后的行为：

- App 请求 `{normalizedBaseUrl}/device`。
- 只有响应中包含有效 `active_url` 时才继续打开激活页。
- 如果 `active_url` 是完整 `http(s)` 地址，则直接打开。
- 如果 `active_url` 是相对路径，则拼接为 `{normalizedBaseUrl}/{path}` 后打开。
- 打开前将激活 URL 的 `lang` 查询参数设为 App 当前语言，保留其他查询参数和片段，与扫描设备入口保持一致。
- 移动端通过 `/web-container?embedded=1` 在当前 WebView 内打开。
- 桌面端通过独立 `WebviewWindow` 打开，主窗口继续停留在 OOD 激活入口。
- 打开成功后关闭输入弹框。

错误处理：

- 地址为空、host 不是有效域名或 IPv4、端口非法时，在弹框内提示并阻止提交。
- `{normalizedBaseUrl}/device` 请求失败、超时、返回非 OOD 数据或缺少有效 `active_url` 时，弹框保持打开并展示错误信息。
- 用户手动输入的远程地址不会触发局域网扫描，也不依赖中继服务、二维码配对或账号云端发现。
...
桌面端的 URL 打开能力以独立窗口为默认产品形态。主 App 中需要打开 URL 的入口，例如 SN 说明页、OOD 设备激活页和设置页调试入口，应通过独立 WebView 窗口承载目标页面，主窗口继续停留在原有业务页面。
...
- 桌面端没有显式传入 label 时，自动生成 `webview_` 前缀的随机 label。
- 桌面端窗口尺寸需要根据主显示器安全计算；默认宽度不小于 1024，默认高度不小于 700，并默认居中显示。
- 桌面端窗口关闭时，如调用方传入关闭回调和 `userData`，需要在窗口销毁后把 `userData` 交回调用方处理。
...
- URL 页面运行在新窗口的 `/web-container` 内部 iframe 中，仍应获得与 Runtime 一致的 `window.BuckyApi` 注入能力。
- 打开 URL 不应改变主窗口 hash 路由，不应影响 DID 创建、导入、设置页或 OOD 扫描页的当前状态。
- 同一业务目标应使用稳定 label 复用窗口；需要允许多开的临时页面可以不传 label，由系统生成随机 label。
- 该能力是 App 内 Runtime 窗口，不等同于调用系统默认浏览器打开网页。
...
| 打开 URL 页面 | URL、可选 title、可选 label、可选窗口参数、可选关闭回调数据 | 桌面端创建或复用独立 `WebviewWindow` 并加载内部 `/web-container`；移动端跳转到当前 WebView 内的 `/web-container?embedded=1` |

手动输入远程 OOD 地址不新增宿主命令。前端负责把用户输入归一化为 OOD 根地址，请求 `{normalizedBaseUrl}/device`，再复用现有打开 URL 页面能力承载 `active_url`。
...
- Active Code：由测试负责人按次提供未使用值，不在仓库文档、截图、日志或报告中记录完整明文。
...
- OOD 入口未绑定状态下，“激活远程设备”按钮可打开地址输入弹框。
- 远程 OOD 地址弹框左侧提供协议下拉框，默认 `http://`，可切换 `https://`；右侧地址输入支持域名、域名加端口、IPv4、IPv4 加端口；未填端口时默认使用 `3182`。
- 远程 OOD 地址确认后会请求归一化地址的 `/device`，并在返回有效 `active_url` 后打开设备激活页。
- 已绑定 SN 的助记词可以通过导入账户恢复。
- 未绑定 SN 的助记词导入失败。
- 设置页能进入身份切换、备份、删除流程。
- 嵌入式 WebView 测试页能调用 `BuckyApi`。
- 桌面端从 SN 说明页、OOD 扫描设备、远程 OOD 地址输入和设置页调试入口打开 URL 时，应创建或聚焦独立 WebView 窗口，主窗口当前页面不被替换。
- 桌面端重复打开同一稳定 label 的 URL 时，应复用已有窗口并聚焦，不重复创建窗口。
...
- 能展示 OOD 入口和绑定状态，能扫描局域网设备，能通过弹框输入远程 OOD 根地址，能打开设备 `active_url`，能使用签名 token 解绑 zone 配置。
- 能读取并展示 App 列表，但不需要实现安装、启动、升级、卸载。
- 能提供嵌入式 iframe 和独立 WebView 容器，并向页面提供兼容 [BuckyApi.md](./BuckyApi.md) 的 `window.BuckyApi`。
- 桌面端打开 URL 时必须使用独立 WebView 窗口承载 `/web-container`，不替换主窗口路由；移动端可复用当前 WebView 的 embedded 容器。
