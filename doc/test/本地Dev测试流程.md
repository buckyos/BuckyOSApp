...
功能用例和 Android 完整操作步骤分别引用 [测试流程](./测试流程.md)、[共用账户与 OOD 生命周期](./端到端/共用账户与OOD生命周期.md) 与 [Android 端到端执行流程](./端到端/Android执行流程.md)，本文档不复制这些步骤。
...
macOS 正式端到端验收不使用本流程。它由运行在目标 Mac 上的 agent 使用 PVE PR 组合包执行，见 [macOS 端到端执行流程](./端到端/macOS执行流程.md)。
...
以下改动需要追加 [Android 端到端执行流程](./端到端/Android执行流程.md) 所引用的共用生命周期相关阶段或全流程：
...
- Windows/macOS 的启动、托盘、窗口或 deep link；此类改动必须执行[桌面 App 单例用例](./测试流程.md#64-windowsmacos-桌面-app-单例)。
